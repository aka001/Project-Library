...
def user1():
    ...
    return dict(ln=ln,temp=temp,name=name,flag=flag,form=form,comment=comment)
def download():
...
def dev():
	...
	if len(c):
		l=max(c)
	cur_file=db((db.file_add.vers>=l)&(db.file_add.project_id==temp)&(db.file_add.fl!=1)&(db.file_add.fl2!=1)).select(db.file_add.ALL)
...
def modify():
	...
	if len(c):
		...
		db((db.file_add.project_id==temp)&(db.file_add.fl2==1)).delete()
	if l==3:
	...
	if l==1:
		db((db.file_add.project_id==temp)&(db.file_add.vers>=l)&(db.file_add.fl==1)).delete()
		db((db.file_add.project_id==temp)&(db.file_add.fl2==1)).delete()
	if l==2:
		db((db.file_add.project_id==temp)&(db.file_add.vers>=l)&(db.file_add.fl==1)).delete()
	if l==3:	
    		...
    		db((db.file_add.project_id==temp)&(db.file_add.vers==2)).update(vers=1)
		db((db.file_add.vers==2)&(db.file_add.project_id==temp)).delete()
    		l=2
	cur_file=db((db.file_add.project_id==temp)&(db.file_add.vers==(l-1))&(db.file_add.fl!=1)&(db.file_add.fl2!=1)).select(db.file_add.ALL)
    	for i in range(0,len(cur_file)):
	    	db.file_add.insert(project_id=temp,Name=cur_file[i]['Name'],new_file=cur_file[i]['new_file'],vers=l,added_by=auth.user_id)
	db((db.file_add.project_id==temp)&(db.file_add.fl==1)).update(fl=0)
	if l==0:
		db((db.file_add.project_id==temp)&(db.file_add.fl3==1)).update(fl3=0)
		db((db.file_add.project_id==temp)&(db.file_add.fl3>=l)).update(vers=l)
	if l==1:
	     	db((db.file_add.project_id==temp)&(db.file_add.fl==1)).update(fl=0)
	     	db((db.file_add.project_id==temp)&(db.file_add.fl3==1)).update(fl3=0)
	     	db((db.file_add.project_id==temp)&(db.file_add.vers>=l)).update(vers=l)
	if l==2:
	     	db((db.file_add.project_id==temp)&(db.file_add.fl==1)).update(fl=0)
	     	db((db.file_add.project_id==temp)&(db.file_add.fl3==1)).update(fl3=0)
	     	db((db.file_add.project_id==temp)&(db.file_add.fl2==1)).update(fl2=0)
	     	db((db.file_add.project_id==temp)&(db.file_add.vers>=l)).update(vers=l)
	redirect(URL('dev',args=temp))
import os
def downm():
    ...
    os.chdir('/home/ashish/web2py/applications/project/uploads')
    ...
    for j in range(0,len(r)):
	    x+=r[j]['new_file']
    os.system('tar cvf '+'out.tar'+' '+x)
    fullpath=os.path.join(request.folder,'uploads','out.tar')
    print fullpath
    return response.stream(fullpath) 
def file_show():
    temp=request.args(0,cast=int)
    temp1=request.args(1)
    cur=db(db.folder.project_id==temp).select(db.folder.ALL)
	...
	l=max(c)+1
    if l==3:
    	files=db((db.file_add.project_id==temp)&(db.file_add.vers==temp1)&(db.file_add.fl3!=1)).select(db.file_add.ALL)
    else:
...
def edit_file():
    ...
    db.file_add.vers.default=c[0]['vers']+1
    db.file_add.project_id.default=temp1
    db.file_add.fl3.default=1
    db.file_add.added_by.default=c[0]['added_by']
    db.file_add.modified_by.default=auth.user_id
    form=SQLFORM(db.file_add)
    if form.process().accepted:
    	db((db.file_add.id==temp)&(db.file_add.fl==1)&(db.file_add.fl3==1)).delete()
    	db(db.file_add.id==temp).update(fl=1)
...
def category():
    temp=request.args(0,cast=int)
    l=db(db.project.id==temp).select(db.project.category)
    fi=db(db.project.category==l[0]['category']).select(db.project.ALL)
    return dict(fi=fi)
